...
public class LineStringLineStringKNNQuery extends KNNQuery<LineString, LineString> {
    private static final Logger LOG = LoggerFactory.getLogger(LineStringLineStringKNNQuery.class);
    ...
    public DataStream<Tuple3<Long, Long, PriorityQueue<Tuple2<LineString, Double>>>> run(DataStream<LineString> lineStringStream, LineString queryLineString, double queryRadius, Integer k) throws IOException {
        ...
        if (this.getQueryConfiguration().getQueryType() == QueryType.RealTime) {
            ...
            return windowBased(lineStringStream, queryLineString, queryRadius, k, uGrid, omegaJoinDurationSeconds, omegaJoinDurationSeconds, allowedLateness, approximateQuery);
        }
...
//                }).startNewChain();
        ...
        var temp = filteredLineStrings.assignTimestampsAndWatermarks(new AscendingTimestampExtractor<LineString>() {
                    @Override
                    public long extractAscendingTimestamp(LineString element) {
                        return element.timeStampMillisec;
                    }
                })
                .keyBy((KeySelector<LineString, String>) lineString -> lineString.gridID);
        var windowedKNN = temp.window(SlidingEventTimeWindows.of(Time.seconds(windowSize), Time.seconds(windowSlideStep))).allowedLateness(Time.seconds(allowedLateness))
//                .trigger(new EveryElementTriggers())
                .apply(new WindowFunction<LineString, PriorityQueue<Tuple2<LineString, Double>>, String, TimeWindow>() {
                    ...
                    public void apply(String gridID, TimeWindow timeWindow, Iterable<LineString> inputTuples, Collector<PriorityQueue<Tuple2<LineString, Double>>> outputStream) throws Exception {
                        ...
                        for (LineString lineString : inputTuples) {
                            double distance;
                            // modified to use DTW distance
                            if (kNNPQ.size() < k) {
                                if (approximateQuery) {
                                    distance = DistanceFunctions.getBBoxBBoxMinEuclideanDistance(queryLineString.boundingBox, lineString.boundingBox);
                                } else {
//                                    distance = DistanceFunctions.getDistance(queryLineString, lineString);
                                    distance = DistanceFunctions.getDTWDistance(queryLineString, lineString, uGrid);
//                                    distance = DistanceFunctions.getFrechetDistance(queryLineString, lineString);
                                }
