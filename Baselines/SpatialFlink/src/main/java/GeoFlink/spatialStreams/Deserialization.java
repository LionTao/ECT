...
public class Deserialization implements Serializable {
    ...
    public static DataStream<Point> TrajectoryStream(DataStream inputStream, String inputType, DateFormat dateFormat, String delimiter,
        ...
        } else if (inputType.equals("MYCSV")) {
            trajectoryStream = inputStream.map(new CSVToTSpatialPoint(uGrid)).returns(Point.class);
        } else {
    ...
    public static DataStream<Point> MyTrajectoryStream(DataStream inputStream, UniformGrid uGrid) {



        DataStream<Point> trajectoryStream = inputStream.map(new CSVToTSpatialPoint(uGrid)).returns(Point.class);
    ...
    public static class CSVToTSpatialPoint extends RichMapFunction<String, Point> {

        UniformGrid uGrid;
        int cut;

        //ctor
        public CSVToTSpatialPoint() {
        }

        public CSVToTSpatialPoint(UniformGrid uGrid) {
            this.uGrid = uGrid;
        }
        ...
        public Point map(String str) throws Exception {
            ...
            // optimized for custom csv format
            // id,ts,lon,lat

            Point spatialPoint;
            List<String> strArrayList = Arrays.asList(str.replace("\"", "").split(",")); // For parsing CSV with , followed by space
            String strOId = strArrayList.get(0);
            long time = Long.valueOf(strArrayList.get(1));
            double x = Double.valueOf(strArrayList.get(2));
            double y = Double.valueOf(strArrayList.get(3));
