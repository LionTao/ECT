...
object TDrive {
  ...
  def main(args: Array[String]): Unit = {
    bench(K = Integer.parseInt(args(3)), windowSize = Integer.parseInt(args(0)), Integer.parseInt(args(1)), Integer.parseInt(args(2)))
  }
  ...
  def bench(K: Int, windowSize: Int, i: Int, step: Int): Unit = {
    ...
    val trajs = spark.sparkContext
      .textFile(
        "/home/liontao/work/stream-distance/notebooks/preprocess/tdrive-dita-long-small.txt")
      .zipWithIndex().map(getTrajectory)
            .filter(_.traj.length >= 40)
