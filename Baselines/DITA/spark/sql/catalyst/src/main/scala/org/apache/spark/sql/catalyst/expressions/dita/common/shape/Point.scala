...
//  }
def minDist(other: Point): Double = {
  val AVERAGE_RADIUS_OF_EARTH_KM = 6371000
  require(coord.length == other.coord.length)
  val latDistance = Math.toRadians(coord(1) - other.coord(1))
  val lngDistance = Math.toRadians(coord(0) - other.coord(0))
  val sinLat = Math.sin(latDistance / 2)
  val sinLng = Math.sin(lngDistance / 2)
  val a = sinLat * sinLat +
    (Math.cos(Math.toRadians(coord(1))) *
      Math.cos(Math.toRadians(other.coord(1))) *
      sinLng * sinLng)
